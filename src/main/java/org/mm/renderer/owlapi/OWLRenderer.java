...
public class OWLRenderer extends ReferenceRendererConfiguration
	...
	// TODO: Add OWLObjectPropertyDeclaration
	@Override
	public Optional<OWLRendering> render(MMExpressionNode mmExpressionNode) throws RendererException {
		...
		} else if (mmExpressionNode.hasOWLObjectPropertyDeclaration()) {
			return renderOWLObjectPropertyDeclaration(mmExpressionNode.getOWLObjectPropertyDeclarationNode());
		} else {
	...
	@Override
	public Optional<OWLRendering> renderOWLObjectPropertyDeclaration(OWLObjectPropertyDeclarationNode objectPropertyDeclarationNode)
			throws RendererException {
		Set<OWLAxiom> axioms = new HashSet<>();
		Optional<OWLObjectPropertyRendering> declaredObjectPropertyRendering = entityRenderer
				.renderOWLObjectProperty(objectPropertyDeclarationNode.getOWLObjectPropertyNode());
		if (!declaredObjectPropertyRendering.isPresent()) {
			return Optional.empty();
		}
		OWLObjectProperty declaredObjectProperty = declaredObjectPropertyRendering.get().getOWLObjectProperty();
		/*
		 * By default, the object property declaration node will produce one OWL object property declaration
		 * axiom
		 */
		OWLDeclarationAxiom declaredAxiom = objectFactory.createOWLDeclarationAxiom(declaredObjectProperty);
		axioms.add(declaredAxiom);

		/*
		 * Add any existing axioms in the object property rendering object
		 */
		axioms.addAll(declaredObjectPropertyRendering.get().getOWLAxioms());
		
		/*
		 * In case the object property declaration has an annotation axiom
		 */
		if (objectPropertyDeclarationNode.hasAnnotationFactNodes()) {
			for (AnnotationFactNode annotationFactNode : objectPropertyDeclarationNode.getAnnotationFactNodes()) {
				OWLAnnotationPropertyNode node = annotationFactNode.getOWLAnnotationPropertyNode();
				Optional<? extends OWLPropertyRendering> propertyRendering = entityRenderer
						.renderOWLAnnotationProperty(node);
				OWLAnnotationValueNode annotationValueNode = annotationFactNode.getOWLAnnotationValueNode();
				Optional<OWLAnnotationValueRendering> annotationValueRendering = entityRenderer
						.renderOWLAnnotationValue(annotationValueNode);
				if (propertyRendering.isPresent() && annotationValueRendering.isPresent()) {
					IRI objectPropertyIri = declaredObjectProperty.getIRI();
					OWLAnnotationProperty property = (OWLAnnotationProperty) propertyRendering.get().getOWLProperty();
					OWLAnnotationValue annotationValue = annotationValueRendering.get().getOWLAnnotationValue();
					OWLAnnotationAssertionAxiom axiom = objectFactory.createOWLAnnotationAssertionAxiom(property,
							objectPropertyIri, annotationValue);
					axioms.add(axiom);
				}
			}
		}
