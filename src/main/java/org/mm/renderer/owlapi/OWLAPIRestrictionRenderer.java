...
import org.mm.renderer.OWLRestrictionRenderer;
...
public class OWLAPIRestrictionRenderer implements OWLRestrictionRenderer
{
	private final OWLOntology ontology;
	private final OWLDataFactory owlDataFactory;
	private final OWLAPICoreRenderer coreRenderer;
	...
	public OWLAPIRestrictionRenderer(OWLOntology ontology, OWLAPICoreRenderer coreRenderer,
			OWLAPIEntityRenderer entityRenderer, OWLAPIClassExpressionRenderer classExpressionRenderer)
	{
			...
			if (this.owlObjectHandler.isOWLObjectProperty(property)) {
				...
				if (objectHasValueNode.isName()) {
					Optional<OWLNamedIndividualRendering> individualRendering = entityRenderer
							.renderOWLNamedIndividual(null); // TODO
					...
					Optional<ReferenceRendering> referenceRendering = coreRenderer
		...
		if (propertyRendering.isPresent()) {
			...
			if (this.owlObjectHandler.isOWLDataProperty(property)) {
				...
				if (hasValueNode.isLiteral()) {
					Optional<OWLLiteralRendering> literalRendering = coreRenderer
			...
			if (this.owlObjectHandler.isOWLObjectProperty(property)) {
				...
				if (objectSomeValuesFromNode.hasOWLClassExpression()) {
					Optional<OWLClassExpressionRendering> classExpressionRendering = classExpressionRenderer
							.renderOWLClassExpression(objectSomeValuesFromNode.getOWLClassExpressionNode());
		...
		if (propertyRendering.isPresent()) {
			...
			if (this.owlObjectHandler.isOWLObjectProperty(property)) {
				...
				if (objectAllValuesFromNode.hasOWLClassExpression()) {
					Optional<OWLClassExpressionRendering> classExpressionRendering = classExpressionRenderer
							.renderOWLClassExpression(objectAllValuesFromNode.getOWLClassExpressionNode());
					if (classExpressionRendering.isPresent()) {
						OWLClassExpression classExpression = classExpressionRendering.get().getOWLClassExpression();
