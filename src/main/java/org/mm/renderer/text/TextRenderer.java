...
public class TextRenderer extends ReferenceRendererConfiguration implements Renderer, ReferenceRenderer,
	...
	@Override
	public Optional<? extends Rendering> renderOWLObjectPropertyDeclaration(
			OWLObjectPropertyDeclarationNode objectPropertyDeclarationNode) throws RendererException {
		StringBuilder textRepresentation = new StringBuilder();
		OWLObjectPropertyNode declaredObjectPropertyNode = objectPropertyDeclarationNode.getOWLObjectPropertyNode();
		Optional<? extends TextRendering> textRenderingResult = null;

		/*
		 * Rendering OWL ObjectProperty
		 */
		textRenderingResult = renderOWLObjectProperty(declaredObjectPropertyNode);
		if (!textRenderingResult.isPresent()) {
			return Optional.empty();
		}
		TextRendering declaredObjectPropertyRendering = textRenderingResult.get();
		textRepresentation.append("ObjectProperty: ");
		textRepresentation.append(declaredObjectPropertyRendering.getRendering());
		if (declaredObjectPropertyRendering.hasComment()) {
			textRepresentation.append(comment(declaredObjectPropertyRendering.getComment()));
		}
		textRepresentation.append(NEWLINE);

		/*
		 * Rendering OWL ObjectProperty's Annotation
		 */
		boolean isFirst = true;
	...
	@Override
	public Optional<? extends TextRendering> renderOWLObjectProperty(OWLObjectPropertyNode objectPropertyNode)
			throws RendererException {
		if (objectPropertyNode.hasReferenceNode()) {
			return renderReference(objectPropertyNode.getReferenceNode());
		} else if (objectPropertyNode.hasNameNode()) {
			return renderName(objectPropertyNode.getNameNode());
		}
		throw new InternalRendererException("Unknown object property node: " + objectPropertyNode.getNodeName());
	}
