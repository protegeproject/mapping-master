...
public class OWLAPIRenderer extends DefaultRenderer implements MappingMasterParserConstants
{
	...
	public OWLAPIRenderer(OWLOntology ontology, OWLDataFactory owlDataFactory, SpreadSheetDataSource dataSource)
	{
	...
	@Override public Rendering renderMMDefaultEntityType(MMDefaultEntityTypeNode defaultEntityTypeNode) throws RendererException
	{
	...
	@Override public Rendering renderMMDefaultValueEncoding(MMDefaultValueEncodingNode defaultValueEncodingNode) throws RendererException
	{
	...
	@Override public Rendering renderMMDefaultPropertyValueType(MMDefaultPropertyValueTypeNode defaultPropertyValueTypeNode) throws RendererException
	{
	...
	@Override public Rendering renderMMDefaultPropertyType(MMDefaultPropertyTypeNode defaultPropertyTypeNode) throws RendererException
	{
	...
	@Override public Rendering renderReference(ReferenceNode referenceNode) throws RendererException
	{
		...
		if (!rendering.nothingRendered())
			rendering.logLine(">>>>>>>>>>>>>>>>>>>> Reference [" + referenceNode.toString() + "] rendered as " + referenceNode
					.getEntityTypeNode() + " " + rendering.getFinalRendering() + " >>>>>>>>>>>>>>>>>>>>");
		else
			rendering
	...
	@Override public Rendering renderOWLIndividualDeclaration(OWLIndividualDeclarationNode individualDeclarationNode) throws RendererException
	{
	...
	@Override public Rendering renderOWLClassDeclaration(OWLClassDeclarationNode classDeclarationNode) throws RendererException
	{
		Rendering rendering = new Rendering();
					...
					for (OWLSubclassOfNode subclassOfNode : classDeclarationNode.getSubclassOfNodes()) {
						for (OWLClassExpressionNode classExpressionNode : subclassOfNode.getClassExpressionNodes()) {
							...
							if (classExpressionRendering.nothingRendered())
								rendering.logLine("processReference: skipping subclass declaration [" + subclassOfNode
										+ "] because of missing class");
							else {
								String classExpressionID = classExpressionRendering.getFinalRendering();
								String declaredClassID = declaredClassRendering.getFinalRendering();
				...
				if (classDeclarationNode.hasAnnotations()) {
					for (AnnotationFactNode annotationFactNode : classDeclarationNode.getAnnotationFactNodes()) {
						...
						if (propertyValueNode.isReference()) {
							...
							String propertyShortName = propertyRendering.getFinalRendering();
						...
						String classShortName = declaredClassRendering.getFinalRendering();
						String propertyShortName = propertyRendering.getFinalRendering();
						...
						String propertyValue = propertyValueRendering.getFinalRendering();
	...
	@Override public Rendering renderOWLClassExpression(OWLClassExpressionNode owlClassExpressionNode) throws RendererException
	{
		if (createEntities)
	...
	@Override public Rendering renderOWLUnionClass(OWLUnionClassNode unionClassNode) throws RendererException
	{
		...
		if (createEntities) {
			...
			for (OWLIntersectionClassNode intersectionClassNode : unionClassNode.getOWLIntersectionClasseNodes()) {
				...
				String classExpressionID = classRendering.getFinalRendering();
	...
	@Override public Rendering renderOWLIntersectionClass(OWLIntersectionClassNode intersectionClassNode) throws RendererException
	{
		Rendering rendering = new Rendering();
	...
	@Override public Rendering renderOWLEnumeratedClass(OWLEnumeratedClassNode enumeratedClassNode) throws RendererException
	{
		Rendering rendering = new Rendering();
		...
		if (createEntities) {
			...
			for (OWLIndividualNode owlIndividualNode : enumeratedClassNode.getOWLIndividualNodes()) {
				...
				String individualShortName = individualRendering.getFinalRendering();
	...
	@Override public Rendering renderOWLClassOrRestriction(OWLClassOrRestrictionNode classOrRestrictionNode) throws RendererException
	{
		Rendering rendering;
	...
	@Override public Rendering renderOWLRestriction(OWLRestrictionNode restrictionNode) throws RendererException
	{
		...
		if (createEntities) {
			if (!propertyNodeRendering.nothingRendered()) {
				String propertyShortName = propertyNodeRendering.getFinalRendering();
				if (this.owlObjectHandler.isOWLDataProperty(propertyShortName)) {
					...
					if (restrictionNode.hasOWLMinCardinality()) {
						...
						if (allValuesFromNode.getOWLAllValuesFromClassNode().hasOWLClassExpression())
							classRendering = renderOWLClassExpression(allValuesFromNode.getOWLAllValuesFromClassNode().getOWLClassExpressionNode());
						else
							classRendering = renderOWLNamedClass(allValuesFromNode.getOWLAllValuesFromClassNode().getOWLNamedClassNode());
						if (!classRendering.nothingRendered()) {
							String classExpressionID = classRendering.getFinalRendering();
						...
						if (someValuesFromNode.getOWLSomeValuesFromClassNode().hasOWLClassExpression())
							classRendering = renderOWLClassExpression(someValuesFromNode.getOWLSomeValuesFromClassNode().getOWLClassExpressionNode());
						else
							classRendering = renderOWLNamedClass(someValuesFromNode.getOWLSomeValuesFromClassNode().getOWLNamedClassNode());
						if (!classRendering.nothingRendered()) {
							String classExpressionID = classRendering.getFinalRendering();
							OWLClassExpression cls = this.owlObjectHandler.getOWLClassExpression(classExpressionID);
			...
			Rendering individualDeclarationRendering, Rendering declaredIndividualRendering) throws RendererException
	{
		for (TypeNode typeNode : individualDeclarationNode.getTypeNodes().getTypeNodes()) {
			...
			String individualShortName = declaredIndividualRendering.getFinalRendering();
			String classShortName = typeRendering.getFinalRendering();
			OWLNamedIndividual individual = this.owlObjectHandler.getOWLNamedIndividual(individualShortName);
			OWLClass cls = this.owlObjectHandler.getOWLClass(classShortName);
			...
			Rendering declaredIndividualRendering) throws RendererException
	{
		for (OWLIndividualNode sameAsIndividualNode : individualDeclarationNode.getSameAsNode().getIndividualNodes()) {
			...
			if (!sameAsIndividualRendering.nothingRendered()) {
				String individual1ShortName = declaredIndividualRendering.getFinalRendering();
				String individual2ShortName = sameAsIndividualRendering.getFinalRendering();
			...
			Rendering declaredIndividualRendering) throws RendererException
	{
		for (OWLIndividualNode differentFromIndividualNode : individualDeclarationNode.getDifferentFromNode()
			...
			if (!differentFromIndividualRendering.nothingRendered()) {
				String individual1ShortName = declaredIndividualRendering.getFinalRendering();
				String individual2ShortName = differentFromIndividualRendering.getFinalRendering();
				OWLNamedIndividual individual1 = this.owlObjectHandler.getOWLNamedIndividual(individual1ShortName);
				OWLNamedIndividual individual2 = this.owlObjectHandler.getOWLNamedIndividual(individual2ShortName);
			...
			throws RendererException
	{
		...
		if (valueExtractionFunctionNode.hasArguments()) {
			for (StringOrReferenceNode argumentNode : valueExtractionFunctionNode.getArgumentNodes()) {
				...
				arguments.add(argumentRendering.getFinalRendering());
			}
		}
	...
	private OWLLiteral getOWLLiteral(OWLPropertyValueNode propertyValueNode) throws RendererException
	{
		...
		if (literalNode.isBoolean())
			...
			throw new RendererException("unknown OWL literal property value " + super.renderOWLPropertyValue(propertyValueNode));
	}
			...
			ReferenceNode referenceNode) throws RendererException
	{
		...
		for (ValueSpecificationItemNode valueSpecificationItemNode : valueSpecificationNode.getValueSpecificationItemNodes()) {
			if (valueSpecificationItemNode.hasStringLiteral())
				...
				referenceTextRendering = renderReference(valueSpecificationItemReferenceNode).getFinalRendering();
				...
				ValueExtractionFunctionNode valueExtractionFunction = valueSpecificationItemNode.getValueExtractionFunctionNode();
			...
			throws RendererException
	{
		if (referenceNode.hasExplicitlySpecifiedTypes()) {
			for (TypeNode typeNode : referenceNode.getTypesNode().getTypeNodes()) {
				Rendering typeRendering = renderType(typeNode);
				if (!typeRendering.nothingRendered()) {
					String typeTextRendering = typeRendering.getFinalRendering();
